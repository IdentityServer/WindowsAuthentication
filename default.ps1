properties {
	...
	$sln_file = "$src_directory\WindowsAuthentication.sln"
	$csproj_file = "$src_directory\WindowsAuthentication\WindowsAuthentication.csproj"
...
task Compile -depends UpdateVersion {
	exec { msbuild /nologo /verbosity:q $csproj_file /p:Configuration=$target_config /p:TargetFrameworkVersion=v4.5 }
}
...
task ILMerge -depends Compile {
	$input_dlls = "$output_directory\IdentityServer.WindowsAuthentication.dll"
	...
	Get-ChildItem -Path $output_directory -Filter *.dll |
		foreach-object {
			...
			if ("$_" -ne "IdentityServer.WindowsAuthentication.dll" -and
			    "$_" -ne "Owin.dll") {
...
task CreateNuGetPackage -depends ILMerge {
	...
	copy-item $src_directory\IdentityServer.WindowsAuthentication.nuspec $dist_directory
	copy-item $output_directory\IdentityServer.WindowsAuthentication.xml $dist_directory\lib\net45\
	exec { . $nuget_path pack $dist_directory\IdentityServer.WindowsAuthentication.nuspec -BasePath $dist_directory -o $dist_directory -version $packageVersion }
}
